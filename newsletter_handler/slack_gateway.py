...
# Bewusst OHNE "bot_message": der Logistik-Deal-Radar-Digest wird von einem Bot
# gepostet und ist genau das, was dieser Handler verarbeiten soll.
...
def fetch_channel_messages(hours: int, latest_hours: int = 0) -> list[dict]:
    """Liest Nachrichten (nur Kanal-Parents) aus #newsletter in einem Zeitfenster.

    Anders als beim objekte-Handler werden Bot-Nachrichten NICHT ausgefiltert –
    der Digest selbst ist bot-authored."""
    ...
    while True:
        params: dict = {
            "channel": config.NEWSLETTER_CHANNEL,
            "oldest": oldest_ts,
            "limit": 100,
        }
        if latest_ts:
    ...
    logger.info("%d Nachrichten aus #newsletter gelesen (letzte %dh)", len(messages), hours)
    return messages
...
def _checkmark_via_reactions_get(ts: str, bot_user_id: str) -> bool:
    try:
        response = httpx.get(
            ...
            headers=_headers(),
            params={"channel": config.NEWSLETTER_CHANNEL, "timestamp": ts, "full": "true"},
            timeout=30.0,
        )
        data = response.json()
        if not data.get("ok"):
...
def add_checkmark(ts: str) -> bool:
    ...
    try:
        response = httpx.post(
            ...
            headers=_headers(),
            json={"channel": config.NEWSLETTER_CHANNEL, "timestamp": ts, "name": config.CHECK_EMOJI},
            timeout=30.0,
        )
        data = response.json()
...
def post_thread_reply(thread_ts: str, text: str) -> bool:
    ...
    try:
        response = httpx.post(
            ...
            headers=_headers(),
            json={"channel": config.NEWSLETTER_CHANNEL, "thread_ts": thread_ts, "text": text},
            timeout=30.0,
        )
        data = response.json()
        if not data.get("ok"):
...
def get_permalink(ts: str) -> str | None:
    try:
        response = httpx.get(
            ...
            headers=_headers(),
            params={"channel": config.NEWSLETTER_CHANNEL, "message_ts": ts},
            timeout=30.0,
        )
        data = response.json()
        if data.get("ok"):
    ...
    return f"https://app.slack.com/archives/{config.NEWSLETTER_CHANNEL}/p{ts.replace('.', '')}"
