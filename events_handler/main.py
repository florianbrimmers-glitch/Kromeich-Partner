...
from .sheets import append_rows, build_row
...
def _process_message(msg: dict, run_id: str, report: RunReport) -> None:
    ...
    for idx, event in enumerate(events):
        ...
        if event.anmeldelink:
            notiz = f"{permalink} | {event.anmeldelink}"
        row = build_row(event, notiz)
        record.zeile = row
        record.in_tabelle = not config.dry_run() and not config.no_write()
        rows_to_write.append(row)
        logger.info("  Event %s#%d: '%s' (%s, %s) -> Zeile vorbereitet",
                    ts, idx, event.event_name, event.datum, event.ort)
        append_record(record)

    # DRY_RUN: erkennen + loggen, aber nicht in die Tabelle schreiben
    if rows_to_write and not config.dry_run():
        written = append_rows(rows_to_write)
        report.zeilen_geschrieben += written
    elif rows_to_write:
        logger.info("[DRY_RUN] %d Zeile(n) NICHT geschrieben (nur vorbereitet)", len(rows_to_write))

    add_checkmark(ts)
    n_events = sum(1 for e in events if e.ist_event)
    if not config.dry_run() and not config.no_write():
        reply = f"{n_events} Event(s) in die Marketing-Event-Tabelle übernommen."
    else:
        reply = f"{n_events} Event(s) erkannt (Dry-Run – noch nicht in die Tabelle geschrieben)."
...
def run_pipeline() -> RunReport:
    ...
    if config.no_write():
        ...
        logger.info("=== DRY RUN – Events werden erkannt/geloggt, aber nicht in die Tabelle geschrieben ===")
...
def _print_summary(report: RunReport) -> None:
    ...
    logger.info("  Zeilen in Tabelle:        %d", report.zeilen_geschrieben)
