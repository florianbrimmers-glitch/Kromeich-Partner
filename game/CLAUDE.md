...
| M13b Mehrere Helden (Gameplay), M14 MP | offen. Taverne zum Anwerben, Armee-Tausch wenn zwei Helden sich treffen, Niederlage erst wenn ALLE Helden gefallen sind (heute entscheidet `_on_battle_defeat` sofort), Wechsel-Knopf in der Kopfzeile. **Jeder "Held dazu"-Pfad MUSS `_recompute_fog_player` und `_recompute_costs` rufen** - siehe `_add_hero` in test_multi_hero.gd. |
