...
| Was noch offen ist (Stand It. 48) | **Braucht eine Entscheidung, weil es Balance oder ein neues System anfasst:** (1) **Artefakte** - das letzte grosse HoMM3-System. Es wuerde die drei Stufe-5-Zauber (`resurrect`, `implosion`, `armageddon`) freischalten, die als Daten bereitliegen: `max_spell_level` erreicht mit Weisheit III nur 4, und laut skills.json braucht Stufe 5 ein Relikt. (2) **Kreatur-Upgrades** (HoMM3s aufgewertete Stufen) - Content-schwer und ein Eingriff in Balance-Pass 10. (3) **Kampfgitter 10x8 -> 8x8**: 80 Zellen a 104 px sind fuer einen Daumen knapp, aber es aendert jede Reichweite und damit die Kampfbalance. (4) **Kampf-Meldungen**: sie tragen noch Kuerzel aus der Entwicklung ("-3 A +120 G +45 XP"); ob knapp oder ausgeschrieben, ist Geschmack. **Ohne Entscheidung machbar, aber nur Kosmetik:** der Rok hat einen rundlichen Rumpf, die beiden Baumwesen des Waldvolks sind generisch. **Dauerhafte Pflicht, unabhaengig davon: jeder "Held dazu"-Pfad MUSS `_recompute_fog_player` und `_recompute_costs` rufen** - siehe `_add_hero` in test_multi_hero.gd und `_on_hire_hero`. |
| Erledigt, war hier mal offen | Armee-Tausch zwischen zwei Helden im Feld (It. 44), Wechsel des aktiven Helden per Tap und ueber das Heldenblatt (M13a/M13b), Taverne als Gebaeude - bewusst verworfen, das Anwerben sitzt im Stadtschirm (M13b). |
