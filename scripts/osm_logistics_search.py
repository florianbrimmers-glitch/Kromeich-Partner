#!/usr/bin/env python3
"""
Sucht Logistik-/Lagerhallen in einem Umkreis um Glandorf via OpenStreetMap (Overpass API)
und filtert nach Grundflaeche (Gebaeude-Footprint).

Nutzung: python3 scripts/osm_logistics_search.py
"""
import json
import math
import time
...
RADIUS_M = 40000          # 40 km
AREA_MIN = 30000          # m2
AREA_MAX = 40000          # m2
# Kontextband (knappe Verfehlungen mit ausgeben)
CONTEXT_MIN = 25000
CONTEXT_MAX = 45000
...
# Gebaeudetypen, die fuer Logistik/Lager relevant sind
QUERY = f"""
[out:json][timeout:180];
(
  way["building"~"warehouse|industrial|logistics|distribution"](around:{RADIUS_M},{CENTER_LAT},{CENTER_LON});
  relation["building"~"warehouse|industrial|logistics|distribution"](around:{RADIUS_M},{CENTER_LAT},{CENTER_LON});
);
out tags geom;
"""
...
def ring_area_m2(coords, ref_lat):
    """Shoelace-Flaeche eines Rings (Liste von {lat,lon}) in m2, lokale equirect. Projektion."""
    if len(coords) < 3:
        return 0.0
    R = 6371000.0
...
def relation_area(el):
    """Multipolygon: Summe Outer-Ringe minus Inner-Ringe (vereinfacht ueber Member-Rollen)."""
    outer = 0.0
    inner = 0.0
    all_lat, all_lon, cnt = 0.0, 0.0, 0
