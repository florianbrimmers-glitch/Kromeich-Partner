...
def _font_verzeichnisse() -> list[str]:
    ...
    2. assets/fonts im Repository – der Fallback für GitHub Actions, wo es
       keinen Skill gibt. Ohne diesen Pfad rendert der Quartalslauf mit
...
def erzeuge_pdf(
    ...
    def _kopf(canv, doc):
        ...
        canv.drawString(rand, 12 * mm,
                        "Kromeich & Partner – Vertraulich, nicht zur Weitergabe")
        canv.restoreState()
    ...
    quellen = {
        config.QUELLE_PROPSTACK:
            ...
            "geführten Mietpreis-Felder je Flächenart. Trägt eine Einheit beides, "
            "gewinnt der ausgeschriebene Mietpreis vor der internen Einschätzung.",
        config.QUELLE_DRIVE:
    ...
    flow.append(Paragraph(
        "<b>Verwendung:</b> die Auswertung ist ein <b>Marktindex</b> und als solcher "
        "verwendbar. Die Konditionen einzelner Objekte stammen aus Mandaten, Beratungen "
        "und Anfragen – sie dürfen nicht herangezogen werden, wenn genau dieses Objekt "
        "angeboten wird.", body,
    ))
